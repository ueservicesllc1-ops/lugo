...
const MezclaMastering = () => {
    ...
    return (
        <div style={{ minHeight: '100vh', backgroundColor: '#0f172a', color: 'white', fontFamily: '"Inter", sans-serif' }}>
            ...
            <section style={{ paddingTop: '160px', paddingBottom: '100px', textAlign: 'center', background: 'radial-gradient(circle at top, rgba(139,92,246,0.1) 0%, transparent 60%)' }}>
                <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '0 20px' }}>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '10px', padding: '8px 20px', borderRadius: '50px', background: 'rgba(0,163,255,0.1)', border: '1px solid rgba(0,163,255,0.2)', color: '#00A3FF', fontSize: '0.8rem', fontWeight: '800', marginBottom: '30px', textTransform: 'uppercase', letterSpacing: '2px' }}>
                        <Zap size={14} fill="#00A3FF" /> {t('worldClassSound')}
                    </div>
                    <h1 style={{ fontSize: '4.5rem', fontWeight: '900', marginBottom: '24px', letterSpacing: '-2.5px', lineHeight: '1' }}>
                        {t('mixMasterHeroTitle')} <span style={{ background: 'linear-gradient(to right, #00A3FF, #00d2d3)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{t('mixMasterHeroTitle2')}</span>
                    </h1>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '25px' }}>
                        {[
                            ...
                            <div key={i} style={{ padding: '35px', background: 'rgba(255,255,255,0.01)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '20px' }}>
                                <div style={{ color: '#00A3FF', marginBottom: '20px' }}>{s.icon}</div>
            ...
            <section style={{ padding: '100px 20px' }}>
                <div style={{ maxWidth: '900px', margin: '0 auto', textAlign: 'center' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px', marginBottom: '60px' }}>
                        ...
                        <div style={{ background: 'rgba(0,163,255,0.05)', padding: '30px', borderRadius: '20px', border: '1px solid rgba(0,163,255,0.1)' }}>
                            <div style={{ color: '#00A3FF', fontWeight: '900', fontSize: '0.8rem', marginBottom: '15px' }}>{t('after')}</div>
                            <button style={{ width: '60px', height: '60px', borderRadius: '50%', background: '#00A3FF', border: 'none', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto', cursor: 'pointer', boxShadow: '0 0 20px rgba(0,163,255,0.4)' }}>
                                <PlayCircle size={30} />
                            </button>
                        </div>
                    </div>
            ...
            {/* CTA */}
            <section style={{ padding: '100px 20px', textAlign: 'center', background: 'linear-gradient(to bottom, transparent, rgba(0,163,255,0.05))' }}>
                <Send size={48} color="#00A3FF" style={{ marginBottom: '30px', opacity: 0.5 }} />
