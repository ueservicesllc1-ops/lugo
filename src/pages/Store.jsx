...
export default function Store() {
    ...
    const getSequencePrice = (song) => {
        ...
        return parseFloat(pricing.wavPrice) || 580.0;
    };
    const getOptionPrice = (song, songField, globalValue, fallback) => {
        const songPrice = parseFloat(song?.[songField]);
        if (Number.isFinite(songPrice) && songPrice > 0) return songPrice;
        if (songField === 'priceMp3') {
            const secondary = parseFloat(song?.priceWavTrack);
        ...
        <div style={{ backgroundColor: '#0f172a', minHeight: '100vh', color: 'white', fontFamily: '"Outfit", sans-serif' }}>
            ...
            {previewSong && (
                <div key="preview-modal" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.88)', backdropFilter: 'blur(12px)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '10px' }}>
                    <div style={{ background: '#020617', width: '100%', maxWidth: '1300px', maxHeight: '95vh', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.08)', overflow: 'hidden', boxShadow: '0 40px 80px rgba(0,0,0,0.8)', color: 'white', display: 'flex', flexDirection: 'column' }}>
                                        ...
                                        <div style={{ marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '20px', background: 'rgba(255,255,255,0.02)', borderRadius: '16px', padding: '15px 20px', border: '1px solid rgba(255,255,255,0.05)', flexShrink: 0 }}>
                                            ...
                                            <div style={{ flex: 1 }}>
                                                ...
                                                <div style={{ height: '4px', background: 'rgba(255,255,255,0.1)', borderRadius: '2px', overflow: 'hidden' }}>
                                                    <div style={{ height: '100%', width: `${Math.max(0, Math.min(100, ((previewProgress - 20) / 60) * 100))}%`, background: '#00A3FF', boxShadow: '0 0 10px #00A3FF', transition: 'width 0.1s linear' }}></div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div style={{ width: '400px', padding: '25px', display: 'flex', flexDirection: 'column', gap: '20px', background: 'rgba(255,255,255,0.01)' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                    {(previewSong.isSingle ? [
                                        ...
                                        { id: 'wav_track', name: 'Pista Instrumental (MP3)', desc: 'Pista de acompañamiento de alta calidad sin voz principal.', price: getOptionPrice(previewSong, 'priceMp3', pricing.mp3Price, 180.0), icon: <Music size={18} /> }
                                    ]).map(opt => (
                                        <div 
                                            key={opt.id}
                                    ...
                                    <button 
                                        onClick={() => {
                                            const opt = (previewSong.isSingle ? [
                                                ...
                                                { id: 'wav_track', name: 'Pista Instrumental (MP3)', price: getOptionPrice(previewSong, 'priceMp3', pricing.mp3Price, 180.0), format: 'MP3' }
                        ...
                        <div style={{ padding: '25px' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                {((selectedSongForOptions.isSingle) ? [
                                    ...
                                    { id: 'wav', name: 'Secuencia (Multitrack)', desc: 'Archivos de audio individuales para Lugo Stage o DAW.', price: getSequencePrice(selectedSongForOptions), format: 'MULTITRACK', icon: <Layers size={18} /> },
                                    ...
                                    { id: 'mp3', name: 'Pista Instrumental (MP3)', desc: 'Pista de acompañamiento de alta calidad sin voz principal.', price: getOptionPrice(selectedSongForOptions, 'priceMp3', pricing.mp3Price, 180.0), format: 'MP3 High Quality', icon: <Music size={18} /> }
                                ]).map((option) => (
                                    <div 
                                        key={option.id}
                ...
                <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '20px', marginTop: '5px' }}>
                            ...
                            <div>
                                <strong style={{ color: '#00A3FF' }}>🎺 Brass & Teclados:</strong> Instrumentos virtuales de gama alta.
                            </div>
                        </div>
                    </div>
